export default {
  ...
  help: {
    t1: 'Это открытая неизменяемая доказуемая бухгалтерская книга; система, которая позволяет прозрачно следить за движениями средств внутри себя',
    t2: 'Свободный доступ к истории транзакций.',
    t3: 'Каждая следующая транзакция несёт на себе отпечаток предыдущей, что является непреодолимым препятствием для изменения содержания последней.',
    t4: 'Транзакции алгоритмически подписываются отправителем, что исключает возможность фальсификации.'
  },
